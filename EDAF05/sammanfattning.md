...
### Devide and Conqure
Devide and conqure is a problem which breaks down a problem into subproblems which are later combined to give the answer to the original problem. 
...
#### Hammilton path
Given a graph G you have to check if there is a way of traversing the graph and visiting each node once. If such path is possible the graph contains a hammilton path.
#### knapsack problem
